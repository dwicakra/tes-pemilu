...
def compile_province_data():
    ...
    compiled_data = {
        "mode": "hhcw",
        "tungsura": {
            "chart": {"progres": {"total": 0, "persen": 0, "progres": 0}},
            "table": {},
        },
        "psu": "Reguler",
        "progres": {"total": 0, "progres": 0},
    ...
    pkwkp_dir = "../pkwkp"
    province_folders = [
        f for f in os.listdir(pkwkp_dir) if os.path.isdir(os.path.join(pkwkp_dir, f))
    ]
    ...
    for province in province_folders:
        ...
        try:
            # Check if province file exists before attempting to open it
            if not os.path.exists(province_file):
                print(f"Skipping {province} - file does not exist.")
                continue
            ...
            compiled_data["tungsura"]["table"][province] = {"psu": "Reguler"}
            ...
            if "tungsura" in province_data and "chart" in province_data["tungsura"]:
                ...
                for key, value in chart_data.items():
                    if key.startswith("1000"):  # Ensure only valid keys are copied
            ...
            print(f"Error decoding JSON for province {province}. Skipping...")
            continue
        except Exception as e:
            print(f"Unexpected error processing province {province}: {e}")
            continue
    ...
    if total_all > 0:
        compiled_data["tungsura"]["chart"]["progres"]["persen"] = round(
            (progres_all / total_all) * 100, 2
        )
    ...
    # sort table by key
    compiled_data["tungsura"]["table"] = dict(
        sorted(compiled_data["tungsura"]["table"].items())
    )
    ...
    try:
        ...
        print(f"Error saving compiled data to {output_file}: {e}")
